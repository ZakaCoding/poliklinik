<?php
    ...
    function errorHandling($errCode, $message)
    {
        ...
        return header('location:' . BASE_URL . 'page/auth/login_admin.php');
        exit();
    }
    ...
    try {
         ...
         */
        $result = $mysqli->query("SELECT * FROM `admins` WHERE `email` = '$email' AND `email_verified_at` IS NOT NULL LIMIT 1");
        ...
        if(password_verify($password,$data['password']))
        {
            ...
            $mysqli->query("UPDATE admins SET remember_token = '$token' WHERE email = '$email'");
